...
local function clstm(inp, inputSize, res)
  ...
  return nn.View(-1,hiddenSize,res,res)(add), nn.View(-1,hiddenSize,res,res)(h2)
end

local function hourglass(n, f, inp)
  -- Upper branch
  local up1 = Residual(f,f)(inp)
  local up2p, up2f = clstm(up1,f,2^(n+2))
  ...
  if n > 1 then low3p, low3f = hourglass(n-1,f,low2)
    ...
    low3p, low3f = clstm(low3p,f,2^(n+1))
  end
  ...
  return nn.CAddTable()({up2p,up3p}), nn.CAddTable()({up2f,up3f})
end
...
function M.createModel(opt, outputDim)
  ...
  local fl_ = nnlib.SpatialConvolution(256,2,1,1,1,1,0,0)(llf)
  local fl = nn.Transpose({2,3},{3,4})(fl_)

  -- Predict pixel value with bilinear sampling
  local rep_ = nn.Replicate(seqLength,2)(inp)
  local rep = nn.Contiguous()(rep_)
  local tran_ = nn.View(-1,3,inputRes,inputRes)(rep)
  local tran = nn.Transpose({2,3},{3,4})(tran_)
  local samp_ = nn.BilinearSamplerBHWD()({tran, fl})
  local samp = nn.Transpose({3,4},{2,3})(samp_)
  ...
  local outi_ = nn.View(-1,seqLength,3,outputRes,outputRes)(samp)
  local outi = nn.SplitTable(-4)(outi_)

  -- Final model
  local model = nn.gModule({inp}, {outp, outi})
