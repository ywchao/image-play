...
function M.createModelRNN(opt)
  ...
  local spl = {}
  local n = 0
  spl[1] = nn.View(-1,2^(n+2),2^(n+2),hiddenSize)(z[1])
  ...
  out[2] = nn.View(1,-1,hiddenSize)(z[1])

  -- Split loss from one mini-batch and sum over c, h, w, and lstm
  local loss_z, c = {}, {}
  local n = 0
  loss_z[1] = nn.Sum(2)(nn.Sum(3)(nn.Sum(4)(loss[1])))
  c[1] = ((2^(n+2))*(2^(n+2))*hiddenSize)
  local coeff = 1/(torch.Tensor(c):sum())
  -- No CAddTable needed for no-skip
  out[#out+1] = nn.MulConstant(coeff)(loss_z[1])
